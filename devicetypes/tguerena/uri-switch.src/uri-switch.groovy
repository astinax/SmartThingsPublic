...
def setLevel(val) {
	...
	if (internal_off_path){
        ...
        def myGroovyMap = ""
        if (input_off_key_1){
        	//myGroovyMap += "${input_off_key_1}=${val}"
            myGroovyMap += "${input_off_key_1}=${"level,${val.toInteger()}"}"
            log.debug myGroovyMap
        }
        if (input_off_key_2){
        	myGroovyMap += "&${input_off_key_2}=${input_off_value_2}"
        }
        ...
        if (input_off_key_4){
        	myGroovyMap += "&${input_off_key_4}=${input_off_value_3}"
        }
        if (myGroovyMap == "" && input_off_value_1 != ""){
        	myGroovyMap += input_off_value_1
        }
